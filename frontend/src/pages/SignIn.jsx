...
const SignIn = () => {
    ...
    const [showPassword, setShowPassword] = useState(false); // For Email Login
    const [showOtpPassword, setShowOtpPassword] = useState(false); // For OTP Login
    ...
    const handleVerifyOtp = async (e) => {
        ...
        if (!formData.password) return alert('Please enter a password'); // Validate password
    ...
    const handleGoogleLogin = () => {
        ...
        // Check for valid Google Client ID
        const isRealId = clientId && clientId.length > 20 && !clientId.includes("YOUR_GOOGLE_CLIENT_ID_HERE");

        if (!isRealId) {
            alert("Google Login is not configured.\nPlease set a valid VITE_GOOGLE_CLIENT_ID in your frontend/.env file and restart the server.");
            return;
        }
                        ...
                        <form onSubmit={phoneStep === 'input' ? handleSendOtp : handleVerifyOtp} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                            <div style={{ position: 'relative' }}>
                                <label style={{ display: 'block', fontSize: '0.9rem', color: 'var(--text-muted)', marginBottom: '0.5rem', fontWeight: '500' }}>
                                    {phoneStep === 'input' ? 'Mobile Number' : 'One Time Password'}
                                </label>
                                <div style={{ display: 'flex', alignItems: 'center', background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border-color)', padding: '0 1rem', transition: 'all 0.3s' }}>
                                    <input
                            ...
                            <button type="submit" style={{ marginTop: '1rem', padding: '1rem', borderRadius: '16px', border: 'none', background: 'var(--primary)', color: 'white', fontWeight: '600', cursor: 'pointer', boxShadow: '0 4px 15px var(--primary-glow)' }}>
                                {isLoading ? 'Processing...' : (phoneStep === 'input' ? 'Send OTP' : 'Verify & Login')}
                            </button>
